...
async function run() {
    ...
    } else if (action === 'sequence' && actionArg) {
        // Format: "click,500,300|type,hello|press,Enter"
        const steps = actionArg.split('|');
        for (const step of steps) {
            const [act, ...args] = step.split(',');
            console.log(`> Sequence: ${act} ${args.join(',')}`);
            ...
            if (act === 'click') {
                ...
                await page.waitForTimeout(parseInt(args[0]) || 500);
            }
